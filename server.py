...
def build_user_prompt(payload):
    ...
    return (
        ...
        "workoutClassification содержит качественную уверенность в типе, не вероятность: manual означает выбор ученика, "
        "high сильные признаки, medium косвенные, low предположение. При needsReview=true не считай тип подтверждением "
        "ключевой работы или доказательством замены задания. Учитывай reasons и limitations. Нагрузка, время и дистанция "
        "таких занятий включены в общие суммы; unconfirmedTypeSessions показывает число неподтвержденных типов, "
        "не включенных в счетчики качественных работ. Не назначай повторную ключевую работу только из-за "
        "неопределенности типа: это не доказательство отсутствия стимула. "
        ...
        "Темповая + легкая длительная в соседние дни допустимы как специфическая связка, но не обязательны каждую неделю. "
        "Сначала отметь полноценные тяжелые беговые стимулы: VO2max, интервалы в усилии 3/5/10 км, пороговую или тяжелую темповую работу, "
        "тяжелые горки, гонку и длительную с быстрым блоком. Между двумя такими стимулами целевой интервал составляет не менее 72 часов. "
        "В календарном плане считай вторник-пятница или среда-суббота обычным 72-часовым вариантом, а среда-пятница - 48-часовым исключением. "
        "Интервал 48-71 час является исключением: используй его только для подготовленного спортсмена с низкой субъективной усталостью, "
        "устойчивой переносимостью подобных недель и ясной целью плотного микроцикла; объясни решение в rationale и сократи объем хотя бы одного стимула. "
        "Меньше 48 часов между полноценными тяжелыми беговыми стимулами не планируй. Учитывай как точки отсчета не только будущие задания, "
        "но и тяжелые тренировки из recentWorkoutsDetailed и уже выполненные дни planningWeek. "
        "Обычная длительная полностью в Z1-Z2 без прогрессии и быстрого финиша не равна второй интенсивной работе, но ее длительность и нагрузку нужно учитывать; "
        "связка тяжелая работа + легкая длительная на следующий день допустима у подготовленного спортсмена, если длительная действительно остается легкой. "
        "Если в одной неделе нужны два близких стимула, например VO2max и интервалы в усилии 10 км, оценивай их суммарный качественный объем: "
        "не назначай одновременно максимальный объем обоих шаблонов и сокращай один из них ради качества выполнения. "
